...
class CaffeineModel {
    ...
    private func addDailyList (newState : CaffeineState) {
        if newState == .Intro{
            ...
            if let firstIndex = dailyList.firstIndex(of: newState.rawValue+String(stateCount[newState]!-1)) {
                dailyList[firstIndex] = newState.rawValue+String(stateCount[newState]!)
            }
            stateCount[newState]! += 1
        }
    }
